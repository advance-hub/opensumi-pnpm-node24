...
- The input is disabled while session creation or Active Session switching owns the editor. During an active stream the main editor remains editable so later turns can be queued, while the Stop action stays visible; session-switch loading must not be presented as a streaming Stop state.
